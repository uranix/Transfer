...
typedef struct {
    ...
    REAL stepsize, minrho, alpha, beta, maxlength;
...
static int addedge(mesh2d *pm, int v1, int v2, int color) {
    #ifdef STRUCTCHECK
    int i;
    for (i=0; i<pm->nE; i++) {
...
static int findpoint(mesh2d *pm, int en) {
    ...
    rp = r/2.0;
    if (pm->stepsize > 0.0) {
	r = pm->stepsize * dist2i(pm->vertex, v1, v2);
    }
...
static int front(mesh2d *pm) {
	...
	if (pm->curfriend[0] == pn) {
	    ...
	    if (addedge(pm, v1, pn, pm->e[en].c) < 0)  return 4; // maxedge
	}
	if (pm->curfriend[1] == pn) {
	    remedge(pm, pm->curfriendlink[1], pm->e[en].c);
	} else {
	    if (addedge(pm, pn, v2, pm->e[en].c) < 0)  return 4; // maxedge
	}
	...
	int *pnC, int *color)
{
    ...
    mesh.maxlength = 0.0;
    for (i=0; i<nE; i++)  {
	if (addedge(&mesh, edge[2*i+0], edge[2*i+1], edgecolor[i]) < 0)  return 4;
...
int mesh2daftss(int *pnV, REAL *vertex, int *pnE, int *edge, int *edgecolor, int *pnT, int *tria, int *triacolor, int nnV, int nnE, int nnT, REAL ss) {
    return mesh2daftss_main(pnV, vertex, pnE, edge, edgecolor, pnT, tria, triacolor, nnV, nnE, nnT, ss, NULL, NULL);
}
int mesh2daft(int *pnV, REAL *vertex, int *pnE, int *edge, int *edgecolor, int *pnT, int *tria, int *triacolor, int nnV, int nnE, int nnT) {
    return mesh2daftss_main(pnV, vertex, pnE, edge, edgecolor, pnT, tria, triacolor, nnV, nnE, nnT, 1.5, NULL, NULL);
}
int mesh2daftfull(int *pnV, REAL *vertex, int *pnE, int *edge, int *edgecolor, int *pnT, int *tria, int *triacolor, int nnV, int nnE, int nnT, REAL ss, int *pnC, int *color) {
    return mesh2daftss_main(pnV, vertex, pnE, edge, edgecolor, pnT, tria, triacolor, nnV, nnE, nnT, ss, pnC, color);
}
