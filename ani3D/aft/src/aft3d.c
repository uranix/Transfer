...
/* obsolete heuristics features */
// CHEATPOINT -- (heuristics) insert internal point in tight spaces (not robust)
//#define CHEATPOINT

// MEGACHEAT -- (heuristics) use advanced version of cheatpoint (still not robust)
//#define MEGACHEAT

// REMOVEPOINTS -- (heuristics) delete some internal points and improve front (not robust)
//#define REMOVEPOINTS
...
static void normvec(REAL *vertex, int v1, int v2, int v3, REAL *px, REAL *py, REAL *pz) { // wrapper for normvec3i() from det.c
    REAL v[3];
...
//	libaft_3d_warn("Repainting %d -> %d", c1, p2);
	pm->color[2*i+1] = p2;
	return pcolor(pm, c1);
    } else if (p1 < p2) {
	for (i=0; i<pm->ncolor; i++) {
...
//	libaft_3d_warn("Repainting %d -> %d", c2, p1);
	pm->color[2*i+1] = p1;
	return pcolor(pm, c2);
    } else return p1;
}

/* Octree functions ****************************************************************************************************************************** OCTREE */
#define TREEDEPTH 24
...
static void remface(mesh3d *pm, int i, int color) {
    ...
    if (pm->npack > pm->nnF) libaft_3d_stop("aft3d.c: Internal error in remface & packface");
    pm->pack[pm->npack] = i;
    pm->npack++;
}
...
//		    libaft_3d_warn("front restart");
	...
	if (addtetra(pm, v[0], v[1], v[2], pn, pm->f[fn].c) < 0)  return -1;
	...
	for (i=0; i<3; i++) {
		...
		addface(pm, v[(i+1)%3], v[(i+2)%3], pn, pm->f[fn].c);
#ifdef STRUCTCHECK
		if (pm->curfriend[i]>=0) {
	...
	REAL ss, double (*f)(double, double, double))	
{
    ...
    mesh.maxarea = 0.0;
    for (i=0; i<nF; i++)  {
	addface(&mesh, face[3*i+0], face[3*i+1], face[3*i+2], facecolor[i]);
