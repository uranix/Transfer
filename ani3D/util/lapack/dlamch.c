/* dlamch.f -- translated by f2c (version 20090411).
...
/*            ( A guess; no known machine ) */
	    ...
	    if ((i__1 = ngpmin - ngnmin, dabs(i__1)) == 1) {
...
/*            ( A guess; no known machine ) */
	...
	} else if ((i__1 = ngpmin - ngnmin, dabs(i__1)) == 1 && gpmin == gnmin)
		 {
...
/*  number, without overflow.  It assumes that EMAX + dabs(EMIN) sum */
...
/*     dabs(EMIN). We then assume that EMAX + dabs(EMIN) will sum */
/*     approximately to the bound that is closest to dabs(EMIN). */
