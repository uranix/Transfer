/* dlasyf.f -- translated by f2c (version 20090411).
...
/*        a 1-by-1 or 2-by-2 pivot block will be used */
	...
	absakk = (d__1 = w[k + kw * w_dim1], dabs(d__1));
...
/*        column K, and COLMAX is its absolute value */
	...
	if (k > 1) {
	    ...
	    colmax = (d__1 = w[imax + kw * w_dim1], dabs(d__1));
	} else {
	    colmax = 0.;
	}
...
/*              element in row IMAX, and ROWMAX is its absolute value */
		...
		jmax = imax + idamax_(&i__1, &w[imax + 1 + (kw - 1) * w_dim1],
			 &c__1);
		rowmax = (d__1 = w[jmax + (kw - 1) * w_dim1], dabs(d__1));
		if (imax > 1) {
		    i__1 = imax - 1;
...
/*                 no interchange, use 1-by-1 pivot block */
		    ...
		    kp = k;
		} else if ((d__1 = w[imax + (kw - 1) * w_dim1], dabs(d__1)) >= 
			alpha * rowmax) {
...
/*        a 1-by-1 or 2-by-2 pivot block will be used */
	...
	absakk = (d__1 = w[k + k * w_dim1], dabs(d__1));
...
/*        column K, and COLMAX is its absolute value */
	...
	if (k < *n) {
	    ...
	    colmax = (d__1 = w[imax + k * w_dim1], dabs(d__1));
	} else {
	    colmax = 0.;
	}
...
/*              element in row IMAX, and ROWMAX is its absolute value */
		...
		jmax = k - 1 + idamax_(&i__1, &w[k + (k + 1) * w_dim1], &c__1)
			;
		rowmax = (d__1 = w[jmax + (k + 1) * w_dim1], dabs(d__1));
		if (imax < *n) {
		    i__1 = *n - imax;
...
/*                 no interchange, use 1-by-1 pivot block */
		    ...
		    kp = k;
		} else if ((d__1 = w[imax + (k + 1) * w_dim1], dabs(d__1)) >= 
			alpha * rowmax) {
