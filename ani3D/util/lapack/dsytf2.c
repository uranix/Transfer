/* dsytf2.f -- translated by f2c (version 20090411).
...
/*        a 1-by-1 or 2-by-2 pivot block will be used */
	...
	absakk = (d__1 = a[k + k * a_dim1], dabs(d__1));
...
/*        column K, and COLMAX is its absolute value */
	...
	if (k > 1) {
	    ...
	    colmax = (d__1 = a[imax + k * a_dim1], dabs(d__1));
	} else {
	    colmax = 0.;
	}
...
/*              element in row IMAX, and ROWMAX is its absolute value */
		...
		jmax = imax + idamax_(&i__1, &a[imax + (imax + 1) * a_dim1], 
			lda);
		rowmax = (d__1 = a[imax + jmax * a_dim1], dabs(d__1));
		if (imax > 1) {
		    i__1 = imax - 1;
...
/*                 no interchange, use 1-by-1 pivot block */
		    ...
		    kp = k;
		} else if ((d__1 = a[imax + imax * a_dim1], dabs(d__1)) >= 
			alpha * rowmax) {
