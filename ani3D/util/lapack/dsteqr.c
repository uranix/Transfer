/* dsteqr.f -- translated by f2c (version 20090411).
...
L10:
	...
	i__1 = nm1;
	for (m = l1; m <= i__1; ++m) {
	    tst = (d__1 = e[m], dabs(d__1));
	    if (tst == 0.) {
		goto L30;
	    }
	    if (tst <= sqrt((d__1 = d__[m], dabs(d__1))) * sqrt((d__2 = d__[m 
		    + 1], dabs(d__2))) * eps) {
		e[m] = 0.;
		goto L30;
	    }
...
L40:
	if (l != lend) {
	    ...
	    for (m = l; m <= i__1; ++m) {
/* Computing 2nd power */
		d__2 = (d__1 = e[m], dabs(d__1));
		tst = d__2 * d__2;
		if (tst <= eps2 * (d__1 = d__[m], dabs(d__1)) * (d__2 = d__[m 
			+ 1], dabs(d__2)) + safmin) {
		    goto L60;
		}
/* L50: */
