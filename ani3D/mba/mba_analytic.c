/* mba_analytic.f -- translated by f2c (version 20090411).
	...
	doublereal *rw, integer *iw, integer *iprint, integer *ierr)
{
    ...
    static icilist io___37 = { 0, message, 0, "(A,I10)", 80, 1 };
    static cilist io___48 = { 0, 0, 0, "(A)", 0 };
    static cilist io___49 = { 0, 0, 0, fmt_5003, 0 };
    static cilist io___50 = { 0, 6, 0, fmt_5003, 0 };
...
/*  The function returns 0 (scalar tensor) or 1 (full tensor).  The */
/*  scalar tensor requires to define only M11. */
...
/* ... runing the basic algorithm for the global grid */
    ...
    ani2_(np, maxp, nf, maxf, ne, maxe, &xyp[4], &iw[iipf], &iw[iipe], nestar,
	    ...
	    , &rw[iqe], &npw, &new__, &rw[ixypw], &rw[ihespw], &rw[irse], (
	    I_fp)metricfunction, &flaganalytic, &flagfile, &chanelout, &
	...
	integer *iw, integer *iprint, integer *ierr)
{
    ...
    static icilist io___55 = { 0, message, 0, "(A,I10)", 80, 1 };
...
/* ================================================================ */
	...
	*ierr = 1001;
	...
	e_wsfi();
	errmes_(ierr, "mbaAnalyticShort", message, (ftnlen)16, (ftnlen)80);
    }
    if (*maxwr <= maxwrav) {
	*ierr = 1002;
	s_wsfi(&io___55);
...
/* ... setup of missing parameter */
    ...
    i__1 = *maxwi - iiend;
    mbaanalytic_(np, maxp, nf, maxf, ne, maxe, &xyp[4], &ipf[4], &ipe[5], &
	    lbf[1], &iw[ilbe], nestar, &npv, &nfv, &nev, &iw[iipv], &iw[iifv],
	     &iw[iiev], &flagauto, status, &maxskipe, &maxqitr, (I_fp)
	    ani_metric_eucl__, &quality, &rquality, maxwr, &i__1, &rw[1], &iw[
	    iiend], iprint, ierr);
/* group (M) */
/* group (Dev) */
/* group (Q) */
